	...
	public Administration(final Management mng,final JFrame frame) throws ParseException {
		...
		addEmpbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Admin.getAdmin().employeeAdd(empNameText.getText(), Integer.parseInt(salaryText.getText()));
		...
		deleteEmployeeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!Admin.getAdmin().getEmployees().isEmpty()) {
					String[] emp1 = new String[Admin.getAdmin().getEmployees().size()];
					int i=0;
					for (Employee em : Admin.getAdmin().getEmployees()) {
						emp1[i]=em.getName();
						i++;
					}
					...
					Delete.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							Admin.getAdmin().employeeDelete(deleteComboBox.getSelectedItem().toString());
							deleteEmployeePanel.setVisible(false);
						}
					});
				}
		...
		stockDisplayButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				foodTable.setModel(new DefaultTableModel(
						Admin.getAdmin().getStock().getFoodString(),
					new String[] {
				...
				drinkTable.setModel(new DefaultTableModel(
						Admin.getAdmin().getStock().getDrinkString(),
					new String[] {
						"Product Name", "Number Of Product", "Entry Date"
					}
				));
				...
				otherTable.setModel(new DefaultTableModel(
						Admin.getAdmin().getStock().getOtherString(),
					new String[] {
						"Product Name", "Number Of Product", "Entry Date"
					}
				));
				...
				addProductButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (foodButton.isSelected()) {
							...
							if (Admin.getAdmin().newOrExistStock(productNameTxt.getText().toString().toLowerCase(), "Food", Integer.parseInt(numberOfProductTxt.getText().toString()))) {
								Admin.getAdmin().stockAdd(productNameTxt.getText().toString().toLowerCase(), "Food", Integer.parseInt(numberOfProductTxt.getText().toString()));
							}
							else {
								Product pr = new Food(productNameTxt.getText().toString().toLowerCase(),Integer.parseInt(numberOfProductTxt.getText().toString()),
										Double.parseDouble(unitPurchasePriceTxt.getText().toString()),Double.parseDouble(unitSalePriceTxt.getText().toString()));
								Admin.getAdmin().addNewProduct(pr, "Food");
							}
					

						}
						else if (drinkButton.isSelected()) {
							if (Admin.getAdmin().newOrExistStock(productNameTxt.getText().toString().toLowerCase(), "Drink", Integer.parseInt(numberOfProductTxt.getText().toString()))) {
								Admin.getAdmin().stockAdd(productNameTxt.getText().toString().toLowerCase(), "Drink", Integer.parseInt(numberOfProductTxt.getText().toString()));
							}
							else {
								Product pr = new Drink(productNameTxt.getText().toString().toLowerCase(),Integer.parseInt(numberOfProductTxt.getText().toString()),
										Double.parseDouble(unitPurchasePriceTxt.getText().toString()),Double.parseDouble(unitSalePriceTxt.getText().toString()));
								Admin.getAdmin().addNewProduct(pr, "Drink");
							}
						}
						else if (otherButton.isSelected()) {
							if (Admin.getAdmin().newOrExistStock(productNameTxt.getText().toString().toLowerCase(), "Other", Integer.parseInt(numberOfProductTxt.getText().toString()))) {
								Admin.getAdmin().stockAdd(productNameTxt.getText().toString().toLowerCase(), "Other", Integer.parseInt(numberOfProductTxt.getText().toString()));
							}
							else {
								Product pr = new Other(productNameTxt.getText().toString().toLowerCase(),Integer.parseInt(numberOfProductTxt.getText().toString()),
										Double.parseDouble(unitPurchasePriceTxt.getText().toString()),Double.parseDouble(unitSalePriceTxt.getText().toString()));
								Admin.getAdmin().addNewProduct(pr, "Other");
							}
						}
						else {
			...
			public void actionPerformed(ActionEvent e) {
				...
				findProductButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (Admin.getAdmin().getStock().getFood().containsKey(deleteProductTxt.getText().toString().toLowerCase())) {
							productName=deleteProductTxt.getText().toString().toLowerCase();
							productType="Food";
							
							slider.setMaximum(Admin.getAdmin().getStock().getFood().get(productName).getNumberOfProduct());
							...
							deleteProductButton.addActionListener(new ActionListener() {
								public void actionPerformed(ActionEvent e) {
									...
									deleteFlag=true;
									if (!sliderFlag && slider.getValue()==Admin.getAdmin().getStock().getFood().get(productName).getNumberOfProduct()) {
										Admin.getAdmin().getStock().getFood().remove(productName);		
									
									}
									else if(!sliderFlag) {
										Admin.getAdmin().getStock().getFood().get(productName).
										setNumberOfProduct(Admin.getAdmin().getStock().getFood().get(productName).getNumberOfProduct()-valueTemp);
									}
									sliderFlag=true;
						...
						else if (Admin.getAdmin().getStock().getDrink().get(deleteProductTxt.getText().toString().toLowerCase())!=null) {
							productName=deleteProductTxt.getText().toString().toLowerCase();
							productType="Drink";
							
							slider.setMaximum(Admin.getAdmin().getStock().getDrink().get(productName).getNumberOfProduct());
							slider.setToolTipText("How many "+ productName+"s do you want to delete.");
							slider.addChangeListener(new ChangeListener() {
								public void stateChanged(ChangeEvent e) {
									deleteFlag=true;
							...
							deleteProductButton.addActionListener(new ActionListener() {
								public void actionPerformed(ActionEvent e) {
									deleteFlag=true;
									if (!sliderFlag && slider.getValue()==Admin.getAdmin().getStock().getDrink().get(productName).getNumberOfProduct()) {
										Admin.getAdmin().getStock().getDrink().remove(productName);								
									}
									else if(!sliderFlag){
										Admin.getAdmin().getStock().getDrink().get(productName).
										setNumberOfProduct(Admin.getAdmin().getStock().getDrink().get(productName).getNumberOfProduct()-valueTemp);
									}
									sliderFlag=true;
									stockDeletePanel.setVisible(false);
									resetStockDelete();
						...
						else if (Admin.getAdmin().getStock().getOther().get(deleteProductTxt.getText().toString().toLowerCase())!=null) {
							productName=deleteProductTxt.getText().toString().toLowerCase();
							productType="Other";
							
							slider.setMaximum(Admin.getAdmin().getStock().getOther().get(productName).getNumberOfProduct());
							slider.setToolTipText("How many "+ productName+"s do you want to delete.");
							slider.addChangeListener(new ChangeListener() {
								public void stateChanged(ChangeEvent e) {
									deleteFlag=true;
							...
							deleteProductButton.addActionListener(new ActionListener() {
								public void actionPerformed(ActionEvent e) {
									deleteFlag=true;
									if (!sliderFlag && slider.getValue()==Admin.getAdmin().getStock().getOther().get(productName).getNumberOfProduct()) {
										Admin.getAdmin().getStock().getOther().remove(productName);								
									}
									else if(!sliderFlag) {
										Admin.getAdmin().getStock().getOther().get(productName).
										setNumberOfProduct(Admin.getAdmin().getStock().getOther().get(productName).getNumberOfProduct()-valueTemp);
									}
									sliderFlag = true;
									stockDeletePanel.setVisible(false);
									resetStockDelete();
			...
			public void actionPerformed(ActionEvent e) {
				...
				showIncomeButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						int count=0;
						for (Map.Entry<String, Double> element : Admin.getAdmin().getDailyIncome().entrySet()) {
						...
						count=0;
						for (Map.Entry<String, Double> element : Admin.getAdmin().getDailyExpense().entrySet()) {
							if (isInInterval(date1,date2,element.getKey())) {
